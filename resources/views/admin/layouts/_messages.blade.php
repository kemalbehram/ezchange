<li class="dropdown messages-menu ">
    <a href="#" class="dropdown-toggle" data-toggle="dropdown" id="dropdownMenuLink" aria-haspopup="true"
    ...
    <ul class="dropdown-menu dropdown-messages mr-auto" aria-labelledby="dropdownMenuLink">
        @if($count>0)
        <li class="dropdown-title">{{ $count }} پیام جدید</li>
        @else
        <li class="dropdown-title">پیام جدیدی وجود ندارد</li>
        @endif
        @isset($notifications)
        @foreach($notifications as $notification)
        @if(\App\Models\User::where('id',$notification->user_id)->exists())
        <li class="unread message dropdown-item">
            <a href="{{ URL::to('admin/emails/'.$notification->id ) }}" class="message"> <i class="float-right"
                    data-toggle="tooltip" data-placement="top" title="Mark as Read"><span class="float-right ol livicon"
                        data-n="adjust" data-s="10" data-c="#287b0b"></span></i>
        ...
        @endisset
        <li class="footer">
            <a href="{{ url('admin/emails/inbox') }}">مشاهده همه</a>
        </li>
    </ul>
</li>
